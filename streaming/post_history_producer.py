...
obj = client.get_object(Bucket='bigdata-4', Key='post_history.csv')
_producer = None
kafka_producer = custom_kafka_producer.connect_kafka_producer()
for line in pd.read_csv(obj['Body'],chunksize =1, sep = '|'):
    time.sleep(1)
    result = line.to_csv(sep = '|',header=False,index=False)
    print(result)
    custom_kafka_producer.publish_message(kafka_producer,'post_history', 'post_history.csv',result)
